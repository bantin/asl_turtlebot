...
class FoodMarkerPublisher():
    def __init__(self):
        ...
        rospy.Subscriber('/delivery_request', String, self.food_request_callback)
                                             
        self.food_goal_pub = rospy.publisher('/nav_pose', Pose2D, queue_size=10) 
        ...
        # Testing 
        self.fun_stuff = ['banana', 'apple', 'orange', 'stop sign', 'bottle']
    ...
    def food_request_callback(self, data):
        request = data.delivery_request
        if request in self.food_dict.keys():
            pose_food = Pose2D()
            pose_food.x = self.food_dict[request][0]
            pose_food.y = self.food_dict[request][1]
            pose_food.theta = self.food_dict[request][2]
